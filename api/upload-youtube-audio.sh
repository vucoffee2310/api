...
# --- Hàm xử lý chính ---
handler() {
  # <<< TOÀN BỘ LOGIC BÊN DƯỚI ĐƯỢC DI CHUYỂN VÀO TRONG HANDLER
  ...
  jq_executable="$base_dir/bin/jq"

  json_body=$(cat)
  if ! video_url=$("$jq_executable" -r '.video_url' <<< "$json_body"); then
    json_error "Invalid JSON body."
  fi

  cookies_content=$("$jq_executable" -r '.cookies' <<< "$json_body")
  extractor_args=$("$jq_executable" -r '.extractor_args' <<< "$json_body")
